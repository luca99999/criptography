...
public class Xoring3 {
	...
	public static void printBitsLong(long a)
	{
		byte bit;
		for(int x = 63; x >= 0; x--)
		{
	...
	public static byte[] longToByte(long val)
	{
		...
		// the casting (byte) takes the right most one
		longByte[7] = (byte)(val >>> 0);
		longByte[6] = (byte)(val >>> 8);
		longByte[5] = (byte)(val >>> 16);
		longByte[4] = (byte)(val >>> 24);
		longByte[3] = (byte)(val >>> 32);
		longByte[2] = (byte)(val >>> 40);
		longByte[1] = (byte)(val >>> 48);
		longByte[0] = (byte)(val >>> 56);
		
		return longByte;

	}
	
	public static long arrayByteToLong(byte[] b) 
	{
		long valore = 0;
		
		// Signed left shift (<<).
		// Shifts the bits of the first operand over by the number of positions 
		// indicated by the second operand so as bto position the byte
		for (int i = 0; i < b.length; i++)
		{
		   valore = (valore << 8) | (b[i] & 0xff);
		}
		
		return valore;
	}
	
	public static void printBitsArray(byte[] b)
	{
	...
	public static void main(String[] args) {
		...
		printBitsArray(byteLong);
		
		long d = arrayByteToLong(byteLong);
